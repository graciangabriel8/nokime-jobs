...
- **From le passe**, because it is what makes the site not shy: the steel band behind the board and the posting form; the rail drawing beside the headline; the steps drawn as a line through three balls; round buttons and chips; the two-tickets-under-a-rail mark; the dotted ring beside each establishment’s name where the distinction goes.
...
An offer is a white card: a 1px `--line` border, radius 12px, no shadow. On a phone it reads top to bottom. First a top line: the kind in a pill, followed by the department when the offer gives one (« Stage · Cuisine »; ink text, 1px `--ink-3` border; a demo adds " · exemple"), the place (town · région) in ink-2, and at the far right the publication date, small in ink-2; where that line is short (a phone, or the left column on a desktop) the place drops under the kind and the date. Then the role as the card's title, linked to its own page for a live offer; the restaurant, with the distinction's place right after its name; the facts in a grid (dates, hours, pay, lodging), each a small label above its value, two columns, four once the box is 520px wide, and a fact the offer does not give is simply not shown (housing reads “Logé” when true, “Non logé” when false, and nothing when absent), except the pay: without it the fact reads “Non communiquée”, with the legal floor for the kind, where there is one, in one ink-2 line under it, on a row of its own; the folded description; and one blue action to write to the establishment (« Écrire à l’établissement »), with the phone as a link beside it when there is one. From 960px the card is one row in three columns: the top line, the role and the restaurant on the left, the facts in the middle, the action on the right, vertically centred; the description folds out under the first two. With no offer (the live state), or none under the chosen filters, the list holds one plain card with the message and the blue button to post an offer.

## Prices, search, « Mise en avant » and the charter

- **Prices.** The establishments' section opens on the free season, one sentence in ink on a steel panel (`.free-banner`), never blue. Below it the four tiers (`.tiers`: Une offre, Brigade, Maison, then the candidates' 0 €) sit in columns under a 1.5px ink rule and split by hairlines: four across on a desktop, two by two on a tablet, one column on a phone. Each tier is its name in ink-2, the price in the headline's face (800, width 85 %), the billing line, one note and a button; only Brigade's button is primary. Under the grid, one line in `.small` (`.vat-note`) carries the 293 B mention: the price shown is the price paid.
- **Filters.** The kind chips and the Logé toggle, then two selects pushed to the right together, the métier (« Tous les métiers », Cuisine, Salle, Hébergement, Spa, always all four) and the région (only the régions that have offers); on a phone each select takes the full width. They combine with each other and with the search.
- **Search.** One input above the filter chips, the inputs' 8px radius and 1.5px `--ink-3` border, a drawn magnifier inside on the left, full width on a phone and at most 560px from there.
- **« Mise en avant ».** A featured offer carries one small label beside its kind: ink text and a 1px ink border, radius 4px, uppercase at 12px, never blue, with its reason spelled out for screen readers. At most three (`MAX_BOOST` in `js/jobs.js`), only from offers stating their pay, hours and housing, from establishments with `charter: true`, and not `boostPaused`. The rule is told under the board, folded behind « Comment la liste est classée ».
- **The charter.** `charte/` is a page title, then the six commitments as a plain list under a 1.5px ink rule, the rail's ball ringed before each (`.pledges`), then the two folded explanations in two columns (`.charter`). It uses no blue beyond the links and focus rings.

## The distinction

A blue rosette, awarded beside an establishment's name, never ranked; a dotted ring while empty. In its own section the rosette is a modest mark (56px, 40px on a phone) set beside the title and aligned with its first line.
